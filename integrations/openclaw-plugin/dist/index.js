...
function isOpenPhoneAndroidNode(ctx) {
    const platform = normalize(ctx.node?.platform);
    const family = normalize(ctx.node?.deviceFamily);
    const commands = ctx.node?.commands ?? [];
    const declaresOpenPhoneCommand = commands.some((command) => normalize(command).startsWith("openphone."));
    const androidLike = platform === "" ||
        platform.includes("android") ||
        family.includes("android") ||
        family.includes("openphone");
    const openPhoneLike = family.includes("openphone") ||
...
async function invokeOpenPhoneNode(ctx) {
