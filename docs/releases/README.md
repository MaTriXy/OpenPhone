...
3. Trigger [release.yml](../../.github/workflows/release.yml) from GitHub
   Actions with a version such as `v0.0.1-preview.1`.
4. The self-hosted Android build runner builds the OTA, stages the artifact,
   generates checksums/manifests, validates the release directory, and uploads
   the OTA plus metadata to GitHub Releases.
5. Release notes should link to device support, flashing notes, known issues,
