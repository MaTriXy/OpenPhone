...
final class SimpleWebSocketClient {
    ...
    private final ExecutorService mWriteExecutor =
            Executors.newSingleThreadExecutor(new ThreadFactory() {
                ...
                public Thread newThread(Runnable runnable) {
                    Thread thread = new Thread(runnable, "OpenPhoneRuntimeWsWriter");
    ...
    private void performHandshake() throws Exception {
        ...
        if (!response.toLowerCase(Locale.US).contains(
            ...
            throw new IOException("websocket accept header missing or unexpected");
        }
    }
    ...
    private String readFrame() throws IOException {
        ...
        if (masked) {
            throw new IOException("websocket server frame was masked");
        }
        if (opcode == 0x0 || (!fin && opcode == 0x1)) {
            throw new IOException("websocket fragmented frames are not supported");
        }
        if (opcode >= 0x8 && (!fin || length > 125)) {
            throw new IOException("websocket invalid control frame");
        }
