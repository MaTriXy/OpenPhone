...
public final class OpenAiRealtimeAdapter implements ModelAdapter {
    ...
    private static String initialTaskPrompt(String userGoal) {
        return "Start this Android phone task and keep working until it is visibly complete "
                ...
                + "something and report later. First call get_screen with include_ui_tree=true and "
    ...
    private static String realtimeInstructions() {
        return "You are OpenPhone Agent, a persistent mobile GUI agent running inside Android. "
                ...
                + "monitoring requests. Use get_screen often. Prefer tap_element over raw tap when an "
                ...
                + "visible; do not proceed into login, signup, install, payment, sending, "
                + "calling, deleting, or purchase surfaces unless the user explicitly asked. "
                + "If credentials, payment, sending, calling, deleting, or risky install is "
                + "required, call ask_user_confirmation or fail_task with a precise reason. "
                + "Do not answer with plain text instead of using tools unless you are blocked. "
